...
class RectangularRoom(object):
    ...
    def clean_tile_at_position(self, pos, capacity):
        ...
        if pos.get_x() > self.width:   # checking if position x value is not out of room range.
            raise ValueError
        if pos.get_y() > self.height:  # checking if position y value is not out of room range.
            raise ValueError
...
class Robot(object):
    ...
    def __init__(self, room, speed, capacity):
        ...
        if speed <= 0:   # speed can not be negative!
            ...
            self.speed = speed
        if capacity < 0:  # capacity can not be negative, robot is not allowed to add dirt to the tile!
            raise ValueError
        else:
...
if __name__=="__main__":
    run_simulation(3, 1.0, 1, 10, 10, 3, 0.9, 1, StandardRobot)
